...
for(j in 1: col) {
  for(k in pc.1@graph@edgeL[j]) {
    cat(paste(k, "\n"),  file=paste0("ant_versions_0",".txt"), append=T)
  }
...
for(j in 1: col) {
  for(k in pc.2@graph@edgeL[j]) {
    cat(paste(k, "\n"),  file=paste0("ant_versions_1",".txt"), append=T)
  }
...
for(j in 1: col) {
  for(k in pc.3@graph@edgeL[j]) {
    cat(paste(k, "\n"),  file=paste0("ant_versions_2",".txt"), append=T)
  }
...
for(j in 1: col) {
  for(k in pc.3@graph@edgeL[j]) {
    cat(paste(k, "\n"),  file=paste0("ant_versions_3",".txt"), append=T)
  }
...
for(j in 1: col) {
  for(k in pc.3@graph@edgeL[j]) {
    cat(paste(k, "\n"),  file=paste0("ant_versions_4",".txt"), append=T)
  }
...
cat(varNames, file="ant_versions_VarNames.txt")
