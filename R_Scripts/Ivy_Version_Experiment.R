# Performs the version experiment for the Ivy SE dataset (please see paper for experimental alg)
library(dplyr)
library(pcalg)
data("ivy-1.1")
col <- ncol(data)
...
for(j in 1: col) {
  for(k in pc.1@graph@edgeL[j]) {
    cat(paste(k, "\n"),  file=paste0("ivy_versions_0",".txt"), append=T)
  }
...
for(j in 1: col) {
  for(k in pc.2@graph@edgeL[j]) {
    cat(paste(k, "\n"),  file=paste0("ivy_versions_1",".txt"), append=T)
  }
...
for(j in 1: col) {
  for(k in pc.3@graph@edgeL[j]) {
    cat(paste(k, "\n"),  file=paste0("ivy_versions_2",".txt"), append=T)
  }
  
}
cat(varNames, file="ivy_versions_VarNames.txt")
